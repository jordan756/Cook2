...
public class CookUpdateMenu extends AppCompatActivity {
    ...
    public void onCreate(Bundle icicle) {
        ...
        dishName = findViewById(R.id.editTextTextPersonName);
        dishCost = findViewById(R.id.editTextTextPersonName2);
        hour = findViewById(R.id.editTextTextPersonName3);
        min = findViewById(R.id.editTextTextPersonName4);
        tags = findViewById(R.id.editTextTextPersonName5);
        createMenuButton = findViewById(R.id.button6);
        ...
        deleteMenuButton = findViewById(R.id.button14);
        ...
        for (Food foodItem: cook.getMenu()) {
            String temp = "Dish: " + foodItem.name + "\n" + "Cost: " + foodItem.cost + "\n"
                    + foodItem.estimatedCookTime.getHours() + "Hr "
                    + foodItem.estimatedCookTime.getMinutes() + "Min" + "\n"
    ...
    private final TextWatcher watcher = new TextWatcher() {
        ...
        public void afterTextChanged(Editable s) {
            if (dishName.getText().toString().length() == 0 ||
                    dishCost.getText().toString().length() == 0 ||
                    hour.getText().toString().length() == 0 ||
                    min.getText().toString().length() == 0 ||
                    tags.getText().toString().length() == 0) {
                createMenuButton.setEnabled(false);
    ...
    public void createMenuItem(View v) {
        ...
        if (duplicate) {
            ...
            Date time = new Date(0, 0, 0, Integer.parseInt(hour.getText().toString()),
                    Integer.parseInt(min.getText().toString()), 0);
            ArrayList<String> tagsList = new ArrayList<String>();
            tagsList.add(tags.getText().toString());
            Food newMenuItem  = new Food(dishName.getText().toString(),
                    Double.parseDouble(dishCost.getText().toString()), time, tagsList);
    ...
    public void cookMenuToMain(View v) {
        finish();
    }
}
